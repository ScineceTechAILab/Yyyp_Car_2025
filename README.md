...
2. **获取代码**:
   - **方法一 (推荐)**: 使用 Git 克隆仓库。
     ```bash
     git clone https://github.com/ScineceTechAILab/YYYP_Car.git
     ```
   - **方法二**: 在 GitHub 页面点击右上角的绿色 `Code` 按钮 -> 选择 `Download ZIP`，下载后解压到本地文件夹。
3. **固件烧录**: 
   - **下载编辑器**: 前往官网下载并安装 [Visual Studio Code](https://code.visualstudio.com/)。
   - **安装插件**: 打开 VSCode，在左侧扩展商店 (Extensions) 搜索 `PlatformIO IDE` 并安装（安装过程可能较慢，请耐心等待完成后重启软件）。
   - **打开项目**: 使用 VSCode 打开刚才下载或解压的项目文件夹。
   - **编译烧录**: 等待左下角加载完毕后，使用 USB 连接主板（确保主板已通电），点击底部状态栏的 ➡️ (Upload) 箭头图标即可自动编译并烧录。
   
   ![固件烧录指南](Files/upload_guide.png)

4. **整车组装**: 打印车模并安装，安装激光传感器等外设。
5. **参数调试**: 开始调参。
